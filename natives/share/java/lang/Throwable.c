...
obj_header **backtrace_object(obj_header *throwable) {
  return &((struct native_Throwable *)throwable)->backtrace;
}
...
DECLARE_NATIVE("java/lang", Throwable, fillInStackTrace,
  ...
  handle *stack_trace = make_handle(
  ...
  for (int j = 0; i >= 0; --i, ++j) {
    ...
    handle *e =
        make_handle(thread, new_object(thread, StackTraceElement));
...
#define E ((struct native_StackTraceElement *)e->obj)
    int line =
        is_frame_native(frame)
            ? -1
            : get_line_number(method->code, frame->plain.program_counter);
...
DECLARE_NATIVE("java/lang", Throwable, getStackTraceElement,
  ...
  obj_header *element =
      *((obj_header **)ArrayData(stack_trace) + index);
