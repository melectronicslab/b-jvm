...
TEST_CASE("Folder in classpath", "[classpath]") {
  ...
  size_t len;
  int ret_val =
      lookup_classpath(&cp, STR("jdk/internal/misc/Unsafe.class"), &bytes, &len);
  REQUIRE(bytes != nullptr);
  REQUIRE(ret_val == 0);
  free(bytes);
  ...
  free(bytes);
  ret_val =
      lookup_classpath(&cp, STR("nested/boi/Boi.class"), &bytes, &len);
  REQUIRE(bytes != nullptr);
  REQUIRE(ret_val == 0);
  free(bytes);
  ret_val = lookup_classpath(&cp, STR("../classpath_test/Chick.class"),
                                  &bytes, &len);
  REQUIRE(bytes == nullptr);
  REQUIRE(ret_val == -1);
  free(bytes);
