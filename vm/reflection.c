...
slice unparse_field_descriptor(slice str,
                                        const field_descriptor *desc) {
  slice write = str;
  // Print '[' repeatedly
...
void reflect_initialize_field(vm_thread *thread,
                                   classdesc *cd,
                                   cp_field *field) {
  classdesc *reflect_Field = thread->vm->cached_classdescs->field;
  handle *field_mirror =
      make_handle(thread, new_object(thread, reflect_Field));
...
#define F ((struct native_Field *)field_mirror->obj)
  ...
  for (int i = 0; i < field->attributes_count; ++i) {
    if (field->attributes[i].kind ==
      ...
      const attribute_runtime_visible_annotations a =
          field->attributes[i].annotations;
      F->annotations =
          CreatePrimitiveArray1D(thread, TYPE_KIND_BYTE, a.length);
...
void reflect_initialize_constructor(vm_thread *thread,
                                         classdesc *cd,
                                         cp_method *method) {
  ...
  handle *result =
      make_handle(thread, (void *)method->reflection_ctor);
#define C ((struct native_Constructor *)result->obj)
  ...
  for (int i = 0; i < method->descriptor->args_count; ++i) {
    ...
    struct native_Class *type = (void *)get_class_mirror(
...
void reflect_initialize_method(vm_thread *thread,
                                    classdesc *cd,
                                    cp_method *method) {
  ...
  handle *result =
      make_handle(thread, new_object(thread, reflect_Method));
...
#define M ((struct native_Method *)result->obj)
  ...
  for (int i = 0; i < method->descriptor->args_count; ++i) {
    ...
    ((void **)ArrayData(M->parameterTypes))[i] = (void *)get_class_mirror(
        thread, load_class_of_field_descriptor(thread, desc));
  }

  slice ret_desc =
      unparse_field_descriptor(str, &method->descriptor->return_type);
  M->returnType = (void *)get_class_mirror(
...
static obj_header * get_method_parameters_impl(vm_thread * thread, cp_method * method, attribute_method_parameters mparams) {
