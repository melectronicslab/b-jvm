...
int jvm_stack_to_wasm_local(compile_ctx *ctx, int index, bjvm_type_kind kind) {
  ...
  if (*l == -1) {
    *VECTOR_PUSH(ctx->wvars, ctx->wvars_count, ctx->wvars_cap) = jvm_type_to_wasm_kind(kind);
    *l = ctx->next_local++;
  }
  return *l;
}
...
expression get_stack_value(compile_ctx *ctx, int index, bjvm_type_kind kind) {
  return bjvm_wasm_local_get(ctx->module, jvm_stack_to_wasm_local(ctx, index, kind),
    jvm_type_to_wasm_kind(kind));
}

expression set_stack_value(compile_ctx *ctx, int index, bjvm_type_kind kind,
                                      expression value) {
  return bjvm_wasm_local_set(ctx->module, jvm_stack_to_wasm_local(ctx, index, kind), value);
}

expression get_local_value(compile_ctx *ctx, int index, bjvm_type_kind kind) {
  return bjvm_wasm_local_get(ctx->module, jvm_local_to_wasm_local(ctx, index, kind),
    jvm_type_to_wasm_kind(kind));
}

expression set_local_value(compile_ctx *ctx, int index, bjvm_type_kind kind,
                                      expression value) {
  return bjvm_wasm_local_set(ctx->module, jvm_local_to_wasm_local(ctx, index, kind), value);
}
...
expression spill_or_load_code(compile_ctx *ctx, int pc, bool do_load, int return_value) {
  ...
  for (int slot = 0; slot < ctx->code->max_locals + ctx->code->max_stack; ++slot) {
    expression frame = bjvm_wasm_local_get(ctx->module, FRAME_PARAM, bjvm_wasm_int32());
    ...
    if (do_load) {
      ...
      expression load = bjvm_wasm_load(ctx->module,
        load_op, frame, 0, offset);
  ...
  if (!do_load) {
    ...
    expression store = bjvm_wasm_store(ctx->module,
      BJVM_WASM_OP_KIND_I32_STORE, frame, pc_sd_const, 0, offsetof(bjvm_stack_frame, program_counter));
    *VECTOR_PUSH(result, result_count, result_cap) = store;
    *VECTOR_PUSH(result, result_count, result_cap) =
      bjvm_wasm_return(ctx->module, bjvm_wasm_i32_const(ctx->module, return_value));
  }

  expression block = bjvm_wasm_block(ctx->module, result, result_count, bjvm_wasm_void());
  return block;
}
...
void add_runtime_imports(compile_ctx *ctx) {
  ...
  ctx->runtime.new_array = bjvm_wasm_import_runtime_function(ctx->module, wasm_runtime_new_array, "iii", "i");
  ctx->runtime.raise_npe = bjvm_wasm_import_runtime_function(ctx->module, wasm_runtime_raise_npe, "i", "i");
...
expression wasm_get_array_length(compile_ctx *ctx, expression array) {
  return bjvm_wasm_load(ctx->module, BJVM_WASM_OP_KIND_I32_LOAD, array, 0, kArrayLengthOffset);
}
...
expression wasm_raise_npe(compile_ctx *ctx) {
  ...
  return bjvm_wasm_return(
    ctx->module,
    bjvm_wasm_call(ctx->module, ctx->runtime.raise_npe, (expression[]){
      bjvm_wasm_local_get(ctx->module, THREAD_PARAM, bjvm_wasm_int32())
    }, 1)
  );
}

expression wasm_raise_oob(compile_ctx *ctx, expression index, expression length) {
  // (return (call $raise_oob (local.get THREAD_PARAM) <index> <length>))
  return bjvm_wasm_return(
    ctx->module,
    bjvm_wasm_call(ctx->module, ctx->runtime.raise_oob, (expression[]){
      bjvm_wasm_local_get(ctx->module, THREAD_PARAM, bjvm_wasm_int32()),
      index,
      length
    }, 3)
...
expression wasm_move_value(compile_ctx *ctx, int pc, int from, int to) {
  ...
  return bjvm_wasm_local_set(ctx->module, jvm_stack_to_wasm_local(ctx, to, kind),
...
expression wasm_lower_array_load_store(compile_ctx *ctx,
  ...
  expression index = get_stack_value(ctx, sd - 1 - is_store, BJVM_TYPE_KIND_INT);
  expression array = get_stack_value(ctx, sd - 2 - is_store, BJVM_TYPE_KIND_REFERENCE);

  // <nullcheck> = (eqz array)
  expression null_check = bjvm_wasm_unop(ctx->module, BJVM_WASM_OP_KIND_I32_EQZ, array);
  // <addrminusheader> = array + index * sizeof(component)
  expression addr = bjvm_wasm_binop(ctx->module, BJVM_WASM_OP_KIND_I32_ADD,
      array,
      bjvm_wasm_binop(ctx->module, BJVM_WASM_OP_KIND_I32_MUL,
        index,
  ...
  if (is_store) {
    ...
    execute = bjvm_wasm_store(ctx->module, BJVM_WASM_OP_KIND_I32_STORE, addr, value, 0, kArrayDataOffset);
  } else {
    expression data = bjvm_wasm_load(ctx->module, BJVM_WASM_OP_KIND_I32_LOAD, addr, 0, kArrayDataOffset);
  ...
  expression bounds_check = bjvm_wasm_binop(ctx->module, BJVM_WASM_OP_KIND_I32_GE_U,
  index, length);
  expression raise_oob = wasm_raise_oob(ctx, index, length);
  // <boundschecked>
  expression bounds_checked = bjvm_wasm_if_else(ctx->module, bounds_check, raise_oob, execute, bjvm_wasm_void());
  // (if (eqz <array>) (<raise npe>) (<boundschecked>)
  expression npe = wasm_raise_npe(ctx);
  expression load_store = bjvm_wasm_if_else(ctx->module, null_check, npe, bounds_checked, bjvm_wasm_void());
  return load_store;
}
...
expression wasm_lower_icmp_lcmp(compile_ctx * ctx,
  ...
  expression zero = is_long ? bjvm_wasm_i64_const(ctx->module, 0) : bjvm_wasm_i32_const(ctx->module, 0);
  expression one = is_long ? bjvm_wasm_i64_const(ctx->module, 1) : bjvm_wasm_i32_const(ctx->module, 1);
  expression negative_one = is_long ? bjvm_wasm_i64_const(ctx->module, -1) : bjvm_wasm_i32_const(ctx->module, -1);

  expression right = get_stack_value(ctx, sd - 1, kind);
  expression left = get_stack_value(ctx, sd - 2, kind);

  bjvm_wasm_binary_op_kind gt = is_long ? BJVM_WASM_OP_KIND_I64_GT_S : BJVM_WASM_OP_KIND_I32_GT_S,
  lt = is_long ? BJVM_WASM_OP_KIND_I64_LT_S : BJVM_WASM_OP_KIND_I32_LT_S;
  ...
  expression result = bjvm_wasm_select(ctx->module, cmp_greater, one,
    bjvm_wasm_select(ctx->module, cmp_less, negative_one, zero));
  return set_stack_value(ctx, sd - 2, BJVM_TYPE_KIND_INT, result);
}

expression wasm_lower_long_shift(compile_ctx *ctx, const bjvm_bytecode_insn *insn, int sd) {
  // Convert int to long
  expression right = get_stack_value(ctx, sd - 1, BJVM_TYPE_KIND_INT);
  expression right_long = bjvm_wasm_unop(ctx->module, BJVM_WASM_OP_KIND_I64_EXTEND_U_I32, right);

  expression left = get_stack_value(ctx, sd - 2, BJVM_TYPE_KIND_LONG);
  bjvm_wasm_binary_op_kind op = insn->kind == bjvm_insn_lshl ? BJVM_WASM_OP_KIND_I64_SHL :
    insn->kind == bjvm_insn_lushr ? BJVM_WASM_OP_KIND_I64_SHR_U : BJVM_WASM_OP_KIND_I64_SHR_S;
...
expression wasm_lower_local_load_store(compile_ctx *ctx, const bjvm_bytecode_insn *insn, int sd) {
  bjvm_type_kind kind;
  bool is_store = false;
...
expression wasm_lower_iinc(compile_ctx *ctx, const bjvm_bytecode_insn *insn, int sd) {
  ...
  expression result = bjvm_wasm_binop(ctx->module, BJVM_WASM_OP_KIND_I32_ADD, local, increment);
  expression iinc = set_local_value(ctx, insn->iinc.index, BJVM_TYPE_KIND_INT, result);
  return iinc;
}

expression wasm_lower_return(compile_ctx *ctx, const bjvm_bytecode_insn *insn, int pc, int sd) {
  // Write the value into memory at "result"
  bjvm_type_kind kind = inspect_value_type(ctx, pc, sd - 1);
  expression value = get_stack_value(ctx, sd - 1, kind);
  expression store = bjvm_wasm_store(ctx->module, BJVM_WASM_OP_KIND_I32_STORE,
    bjvm_wasm_local_get(ctx->module, RESULT_PARAM, bjvm_wasm_int32()), value, 0, 0);
  return store;
}
...
expression converted = bjvm_wasm_unop(ctx->module, BJVM_WASM_OP_KIND_##op, value); \
...
expression right = get_stack_value(ctx, sd - 1, kind); \
expression left = get_stack_value(ctx, sd - 2, kind); \
expression result = bjvm_wasm_binop(ctx->module, BJVM_WASM_OP_KIND_##op, left, right); \
...
break; \
  ...
  for (int i = 0, pc = bb->start_index; i < bb->insn_count; ++i, ++pc, ++expr_i) {
    ...
    case bjvm_insn_arraylength: {
      ...
      expression nullcheck = bjvm_wasm_unop(ctx->module, BJVM_WASM_OP_KIND_I32_EQZ, array);
      expression raise_npe = wasm_raise_npe(ctx);
      expression length = wasm_get_array_length(ctx, array);
      expression store_length = set_stack_value(ctx, sd - 1, BJVM_TYPE_KIND_INT, length);
      PUSH_EXPR = bjvm_wasm_if_else(ctx->module, nullcheck, raise_npe, store_length, bjvm_wasm_int32());
      break;
    }
    case bjvm_insn_athrow: {
      expression exception = get_stack_value(ctx, sd - 1, BJVM_TYPE_KIND_REFERENCE);
      int store_offs = offsetof(bjvm_thread, current_exception);
      expression store = bjvm_wasm_store(ctx->module, BJVM_WASM_OP_KIND_I32_STORE,
        thread(ctx), exception, 0, store_offs);
      expression execute = bjvm_wasm_if_else(ctx->module,
        bjvm_wasm_unop(ctx->module, BJVM_WASM_OP_KIND_I32_EQZ, exception),
        wasm_raise_npe(ctx), store, bjvm_wasm_void());
      PUSH_EXPR = execute;
      expression ret = bjvm_wasm_return(ctx->module, bjvm_wasm_i32_const(ctx->module, BJVM_INTERP_RESULT_EXC));
      PUSH_EXPR = ret;
      break;
    }

    case bjvm_insn_d2f:
      CONVERSION_OP(BJVM_TYPE_KIND_DOUBLE, BJVM_TYPE_KIND_FLOAT, F32_DEMOTE_F64);
    case bjvm_insn_d2i:
      CONVERSION_OP(BJVM_TYPE_KIND_DOUBLE, BJVM_TYPE_KIND_INT, I32_TRUNC_SAT_F64_S);
    case bjvm_insn_d2l:
      CONVERSION_OP(BJVM_TYPE_KIND_DOUBLE, BJVM_TYPE_KIND_LONG, I64_TRUNC_SAT_F64_S);
    case bjvm_insn_dadd:
    ...
    case bjvm_insn_lreturn: {
      ...
      PUSH_EXPR = bjvm_wasm_return(ctx->module, bjvm_wasm_i32_const(ctx->module, BJVM_INTERP_RESULT_OK));
      break;
    }
    case bjvm_insn_dsub:
    ...
    case bjvm_insn_dup2_x2:
      ...
      break;
    case bjvm_insn_f2d:
      CONVERSION_OP(BJVM_TYPE_KIND_FLOAT, BJVM_TYPE_KIND_DOUBLE, F64_PROMOTE_F32);
    case bjvm_insn_f2i:
      CONVERSION_OP(BJVM_TYPE_KIND_FLOAT, BJVM_TYPE_KIND_INT, I32_TRUNC_SAT_F32_S);
    case bjvm_insn_f2l:
      CONVERSION_OP(BJVM_TYPE_KIND_FLOAT, BJVM_TYPE_KIND_LONG, I64_TRUNC_SAT_F32_S);
    case bjvm_insn_fadd:
    ...
    case bjvm_insn_i2c: {
      ...
      expression result = bjvm_wasm_binop(ctx->module, BJVM_WASM_OP_KIND_I32_AND, val, max_char);
      PUSH_EXPR = set_stack_value(ctx, sd - 1, BJVM_TYPE_KIND_INT, result);
      break;
    }
    case bjvm_insn_i2d:
      CONVERSION_OP(BJVM_TYPE_KIND_INT, BJVM_TYPE_KIND_DOUBLE, F64_CONVERT_S_I32);
    case bjvm_insn_i2f:
      CONVERSION_OP(BJVM_TYPE_KIND_INT, BJVM_TYPE_KIND_FLOAT, F32_CONVERT_S_I32);
    case bjvm_insn_i2l:
    ...
    case bjvm_insn_l2d:
      CONVERSION_OP(BJVM_TYPE_KIND_LONG, BJVM_TYPE_KIND_DOUBLE, F64_CONVERT_S_I64);
    case bjvm_insn_l2f:
      CONVERSION_OP(BJVM_TYPE_KIND_LONG, BJVM_TYPE_KIND_FLOAT, F32_CONVERT_S_I64);
    case bjvm_insn_l2i:
    ...
    case bjvm_insn_iconst: {
      expression value = bjvm_wasm_i32_const(ctx->module, (int)insn->integer_imm);
  ...
  expression block = bjvm_wasm_block(ctx->module, results, result_count, bjvm_wasm_void());
  ...
  if (b->is_loop_header) {
    ...
    while (true) {
      ...
      for (int i = 0; i < b->next_count; ++i) {
        int next = b->next[i];
        if (bjvm_query_dominance(b, analy->blocks + next) && !ctx->visited[next]) {
...
static bjvm_wasm_expression *recurse(bjvm_code_analysis *analy, topo_ctx *topo) {
...
bjvm_wasm_jit_compiled_method* bjvm_wasm_jit_compile(bjvm_thread *thread,
  ...
  // Credit to https://medium.com/leaningtech/solving-the-structured-control-flow-problem-once-and-for-all-5123117b1ee2
