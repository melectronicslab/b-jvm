...
static bjvm_cp_method* get_unambiguous_method(bjvm_itable_method_t m) {
  return (bjvm_cp_method*) (m & ~(BJVM_ITABLE_METHOD_BIT_INVALID | BJVM_ITABLE_METHOD_BIT_AMBIGUOUS));
}
...
static bjvm_itable copy_itable(const bjvm_itable *src) {
  ...
  memcpy(result.methods, src->methods, src->method_count * sizeof(bjvm_cp_method *));
  return result;
}
...
static void merge_itable(bjvm_itable *dst,
  const bjvm_itable *src, bjvm_string_hash_table poisoned, bjvm_classdesc *classdesc) {
  ...
  for (int i = 0; i < src->method_count; ++i) {
    ...
    bjvm_cp_method *maybe_overrides =
      bjvm_easy_method_lookup(classdesc, d_method->name, d_method->unparsed_descriptor, true, false);
...
static void setup_itables(bjvm_classdesc *super, bjvm_classdesc *classdesc) {
  ...
  for (int iface_i = 0; iface_i <= classdesc->interfaces_count; ++iface_i) {
    ...
    for (int itable_i = 0; itable_i < iface->itables.interface_count; ++itable_i) {
      bjvm_itable *super_itable = iface->itables.entries + itable_i;
      for (int method_i = 0; method_i < super_itable->method_count; ++method_i) {
        ...
        void* existing = bjvm_hash_table_lookup(&discovered, ident.chars, ident.len);
        if ((existing && existing != underlying) || is_ambiguous(ref)) {
          // Ambiguity across superinterfaces :o
          (void)bjvm_hash_table_insert(&ambiguous, ident.chars, ident.len, (void*)1);
        } else if (existing == nullptr) {
          (void)bjvm_hash_table_insert(&discovered, ident.chars, ident.len, underlying);
        }
      }
    }
  }
  ...
  // rules, which prioritise superinterfaces.
  for (int iface_i = 0; iface_i <= classdesc->interfaces_count; ++iface_i) {
    bjvm_classdesc *iface;
    if (iface_i < classdesc->interfaces_count)
      iface = classdesc->interfaces[iface_i]->classdesc;
    ...
    for (int itable_i = 0; itable_i < iface->itables.interface_count; ++itable_i) {
      ...
      *VECTOR_PUSH(itables->interfaces, itables->interface_count, itables->interface_cap) = super_itable->interface;
      merge_with = VECTOR_PUSH(itables->entries, itables->entries_count, itables->entries_cap);
  ...
  if (classdesc->access_flags & BJVM_ACCESS_INTERFACE) {
    ...
    for (int i = 0; i < classdesc->methods_count; ++i) {
      ...
      if (itable_include(method)) {
        ...
        *VECTOR_PUSH(itable.methods, itable.method_count, itable.method_cap) = make(method);
      }
    }
    bjvm_itables *itables = &classdesc->itables;
    *VECTOR_PUSH(itables->interfaces,
      itables->interface_count,
      itables->interface_cap) = classdesc;
    *VECTOR_PUSH(itables->entries,
      itables->entries_count,
...
bjvm_cp_method * bjvm_vtable_lookup(bjvm_classdesc *classdesc, int index) {
  assert(index >= 0 && index < classdesc->vtable.method_count && "vtable index out of range");
...
bjvm_cp_method * bjvm_itable_lookup(bjvm_classdesc *classdesc,
  ...
  for (int i = 0; i < classdesc->itables.interface_count; ++i) {
    if (classdesc->itables.interfaces[i] == interface) {
      ...
      assert(index >= 0 && index < itable.method_count && "itable index out of range");
